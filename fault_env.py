...
class FaultEnv():
    ...
    def __init__(self, props):
        """Initialize the FaultEnv with the properties.
        ...
        self.props = props
        self.init_topo()
        self.simulator = Simulator(props, self.num_nodes)
    ...
    def init_topo(self):
        ...
        # print(self.commodities_to_paths)
    ...
    def plot_traffic_matrix(self, traffic_matrix, title="Traffic Matrix"):
        """绘制流量矩阵的热图。
        ...
        plt.xticks(ticks=np.arange(self.num_nodes), labels=np.arange(self.num_nodes))
        plt.yticks(ticks=np.arange(self.num_nodes), labels=np.arange(self.num_nodes))
        plt.grid(False)
        plt.show()
