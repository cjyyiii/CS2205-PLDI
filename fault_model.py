...
class FaultModel():
    """Fault class for training and testing the model."""
    def __init__(self, props, env, device):
        """Initialize the Fault with the properties, environment, model and device.

        Args:
            props: arguments from the command line
            env: environment for the Fault
            model: model for the Fault
            device: GPU or CPU
        """
        self.props = props
        self.env = env
    ...
    def test(self, test_dl, model, device):
        ...
        """
        model = model.to(device)
        result_save_path = os.path.join(RESULT_DIR, self.props.topo_name, 'Fault', 'result.txt')
        split_save_path = os.path.join(RESULT_DIR, self.props.topo_name, 'Fault', 'split.txt')
        model.eval()
        with torch.no_grad():
            with tqdm(test_dl) as tests:
                ...
                for inputs, targets in tests:
                    ...
                    paths_over_total = self.commodities_to_paths.transpose(0, 1).matmul(1.0 / commodity_total_weight) #shape: (num_paths, 1)
                    split_ratios = paths_weight.mul(paths_over_total) #shape: (num_paths, 1)
                    split_ratios=split_ratios.cpu().numpy()
                    test_losses.append(loss_val)
        
        print_to_txt(split_ratios, split_save_path)
        print_to_txt(test_losses, result_save_path)

class FaultDataset(Dataset):
    """Dataset for the FaultNetWork."""
